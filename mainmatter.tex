...
First, articles are pre-classified into one of the \href{https://msc2020.org}{{63}} subjects between 00-XX (General) and 97-XX (Mathematics Education).
...
In \citeyear{BarthelTB13} \citeauthor{BarthelTB13} performed automated
subject classification for {\color{red} parts of the (?)} the zbMATH corpus~\cite{BarthelTB13}.
...
The proposed method was implemented and is currently being used
especially to pre-classify general journals {\color{red} [+ Referenz auf Zbl
  1296.01048 (?)]} with few other information like references.
For a majority of journals, coarse- and fine-grained codes can be
found by statistically analyzing the MSC codes from referenced documents matched within the zbMATH corpus.
...
For accessible exploration, we have made the best-performing approaches available in our \emph{AutoMSC} implementation and have shared our code on our website. %\footnote{\url{https://github.com/ag-gipp/20CicmAutoMsc}}.
We hope that in the future, other application domains requiring an accurate labeling of publications into their respective Mathematics Subject Classification can benefit from this work, since AutoMSC delivers comparable results to human experts in the first level of MSC labeling, all without requiring manual labor or trained experts.
In future, zbMATH will use our new method for all journals for which the method by~\citeauthor{SchonebergS14}~\cite{SchonebergS14} has been used since 2014.
