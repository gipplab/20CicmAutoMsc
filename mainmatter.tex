...
However, we did not evaluate advanced encodings of mathematical formulae. This is subject to future work, cf. Figure~1.
...
%\item
%How well do automatic methods perform in comparison to a human baseline?
Third we evaluated the effect of POS-preprocessing~\cite{SchonebergS14}.
We found that modern machine learning methods do not benefit from the POS tagging based model developed by \cite{SchonebergS14}, cf. Figure~2.


Fourth we evaluated which features are most important for the accurate classification.
We conclude that references have the highest prediction power, followed by abstract text and title.
...
However, the self-reported score can be used to reduce the classification effort by 86.2\%, without a loss in classification quality.
%\end{enumerate}
...
Thereafter, we will re-investigate the impact of mathematical formulae and since the inherently combined representation of text and formulae was not successful.
